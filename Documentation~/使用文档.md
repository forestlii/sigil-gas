...
10. [近战战斗](#10-近战战斗)
11. [移动 / Locomotion](#11-移动--locomotion)
...
> 📦 **移动在独立配套包** `com.likeon.gas.movement`（不在核心包内）。movement 是状态总线的*消费方*、非能力系统本身，故拆出让 GAS 核心专注。命名空间不变（`Likeon.GAS`），装上配套包即可用下文 API；核心可单独配你自己的移动方案。

`CharacterMovementSystemComponent`（需要 `CharacterController`，**来自配套包**）。数据驱动 + 状态总线对接 GAS。

```csharp
var move = gameObject.AddComponent<CharacterMovementSystemComponent>();
// 同物体有 ASC 时，Awake 自动把 ASC 设为标签 provider —— 移动状态会镜像到 ASC
...
`MovementDefinition`（*Create → Likeon → GAS → Movement Definition*）：含若干 MovementSet，每个 Set 下有 Walk/Jog/Sprint 的 `MovementStateSetting`（Speed/Acceleration/...）。可栈式切换（装备大剑 push 一套）。

**状态总线**：调 `SetDesiredMovement(Sprint)` 后，`Movement.State.Sprint` 会自动镜像到 ASC 的松散标签——于是[输入层](#9-输入分发状态驱动按键多态)的"冲刺时滑铲"就接上了真实移动状态。

> **运动动画**：框架提供 `LocomotionAnimationDriver` 数据驱动层 + 示例分层 Controller 生成器，详见下方 11.1。（传统逐节点动画图方案在 Unity 无直接等价物，故用 Animator 数据驱动层替代；动画 clip 由宿主工程填。）
...
// 或它写 Animator 参数（Speed/Direction/VelocityYaw/LeanForward/LeanRight/Grounded/
//   Jumping/Falling/ViewYaw/ViewPitch/PitchAmount）喂混合树。
```
...
当前版本为**单机权威逻辑**。网络复制 / 预测未实现，计划在 **v2** 用 Netcode for GameObjects 重写。所有"会改状态"的入口集中在 `AbilitySystemComponent`，便于届时统一接 authority 判断。
