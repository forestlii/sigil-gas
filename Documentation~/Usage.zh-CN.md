...
> 实现现状（单机版）：`AbilityTagsToCancel` 与（规则/技能上的）`ActivationRequiredTags`/`ActivationBlockedTags` 已在激活流程中强制生效；`AbilityTagsToBlock` 字段已收集但当前未强制（"激活期间挡住别的技能"建议用标签实现：技能 A 在 `ActivationOwnedLooseTags` 挂 `State.Casting`，技能 B 的 `ActivationBlockedTags` 含 `State.Casting`）。
