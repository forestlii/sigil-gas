...
`AbilityInteractionRules`（*Create → Likeon → GAS → Ability Interaction Rules*）把"技能之间 block/cancel/激活准入"数据化：每条 `AbilityTagRule` 含 `AbilityTag`（规则作用于哪些技能）+ `AbilityTagsToBlock` / `AbilityTagsToCancel`（本技能激活时挡/取消带这些标签的技能）+ `ActivationRequiredTags` / `ActivationBlockedTags`（激活准入）。其中**条件规则** `ConditionalAbilityTagRules` 带 `ActorTagQuery`（`GameplayTagQuery`），仅当角色当前标签满足查询时该组规则才生效（状态感知）。挂到 ASC：
...
### 10.5 武器与技能（不同武器配不同技能）

⚠️ **先纠正一个常见预设**：`WeaponComponent` **本身不持有、也不授予 ability 资产**——它没有"技能列表"字段。装备时它只把自己的 `weaponTags`（如 `Weapon.Sword`）作为松散标签**注入持有者 ASC**（`applyWeaponTagsToOwner`），卸下时移除。所以"切武器换技能"是**靠标签**实现的，有两条路：

**路 A（框架内建·标签门控，推荐，零代码）**：技能一次性全授予 ASC，每把武器的技能在技能资产里设 `ActivationRequiredTags = [Weapon.Sword]`。装上剑 → `Weapon.Sword` 标签在 → 只有剑的技能能激活；换刀 → 剑标签移除、刀标签注入 → 自动切到刀的技能。同一个攻击键（`InputTag.Attack`）因此在不同武器下激活不同技能（配合 9.2 的多态：给每把武器一个排前的 `InputProcessor_ActivateAbilityByTag`，`StateQuery = 含对应武器标签`）。

**路 B（装备时授予/回收，需自己写胶水）**：若你想让武器**带着**自己的技能资产（装上才授予、卸下就回收），框架没有现成字段——订阅 `WeaponComponent.OnEquipped` / `OnUnequipped` 自己调：

```csharp
weapon.OnEquipped   += owner => _handles = ownerASC.GrantLoadout(swordLoadout); // AbilityLoadout 资产
weapon.OnUnequipped += ()    => _handles.RevokeFrom(ownerASC);                  // GrantedAbilityHandles.RevokeFrom
```

> 选型：单纯"按当前武器门控已授予技能"用路 A；"技能随武器动态增删（技能栏也跟着变）"用路 B。两者可混用。

**`WeaponComponent` / `IWeapon` 其它能力：**

- **`SourceObject`**（`Object`，可读写）—— 武器背靠的装备实例或数据资产。武器本身只注入标签，`SourceObject` 是给装备 / 掉落 / 数据表系统"从武器溯源回去"的反向引用。纯粹给你读，框架不解释它。
- **瞄准开关** —— `SetTargeting(bool)` / `ToggleTargeting()` / `IsTargeting` + `OnTargetingChanged`，是武器层的瞄准/开火标志，**区别于锁定系统**（`TargetingSystemComponent`）。卸下武器时复位为 `false`。订阅 `OnTargetingChanged` 可切准星、FOV 或瞄准动画。
- **多套判定段** —— 除主判定 `MeleeTrace`（+ 其下标）外，往 `AdditionalTraces` 加额外的 `(MeleeAttackTrace, entryIndex)` 对。`SetWeaponActive(true/false)` 会把主判定**和**所有额外段一起开/关（双刃 / 主副多段命中盒）。`RefreshTraceInstances()` 把每段的来源重指向当前 owner ASC（`Equip` 时自动调）。
...
> 💡 **排查前先开调试器**：`Likeon ▸ GAS ▸ GAS Debugger`（[§13.1](#131-运行时调试器-gas-debugger)）选中出问题的角色——下面大多数问题它一眼就能定位。
...
| AbilityTriggers（事件标签触发技能） | `ActivateOnGranted` + `AbilityTask_WaitGameplayEvent` | 常驻监听模式，见 §18.3 |
...
| Gameplay Debugger（Shift+'） | `Likeon ▸ GAS ▸ GAS Debugger`（§13.1） | 选中任意 GameObject 检视 |
