...
- **技能激活现在是重入安全的（激活临界区锁）。** `GameplayAbility.IsActive` 要到 `Activate()` 内才置位，而它跑在 `TryActivateSpec` 里取消/阻挡回调（激活组取消、TagRelationship 取消 → 被取消技能的 `OnEndAbility` 用户代码）**之后**。从这些回调里对**同一句柄**重入 `TryActivateAbility` 时 `IsActive` 仍为 false、绕过守卫 → 双激活（激活期松散标签挂两遍、`OnActivateAbility` 与 `OnAbilityActivated` 各发两遍）。新增 `GameplayAbilitySpec.IsActivating` 临界区标志把这段窗口挡住；重入激活按 `AlreadyActive` 拒绝，对齐 UE 激活期的重入保护。技能自身在激活中施加的效果（消耗/冷却）**仍立即生效**——这是 per-spec 临界区锁、不是效果延迟队列，刻意不延迟它们。*(重入安全 3 阶段中的第 2 阶段——输入(C6/C7)待后续。)*
- **输入分发与缓冲窗口现在是重入安全的（第 3 阶段——C6/C7）。** 两处修复：**(C7)** `InputControlSetup.HandleInput` 原来遍历 `FilterInputProcessors` 返回的**共享** `_filterScratch`；某处理器在自己的 `HandleInput` 里重新分发输入时，内层会 `Clear()` 掉外层正在遍历的同一 List → 抛 `InvalidOperationException`。改为每次调用从借还池借一个独立 List（稳态零分配），嵌套分发各用各的表。**(C6)** `FireBufferedInput` 收尾原来关掉**所有**活跃缓冲窗口——包括分发过程中某处理器刚开的下一段连招窗口 → 连招预输入链断；现在只快照进入时的窗口、只关这些，分发中新开的窗口保留。且 `CloseInputBufferWindow` 不再在 `FireBufferedInput` 已关掉该窗口时重复派发一次 `(tag,false)` 关闭事件。至此三阶段重入安全（效果、技能激活、输入）全部完成。*注：* `InputControlSetup.AddChecker` / `AddProcessor` 会改动资产的序列化列表、已文档标注仅初始化期可调——运行时要改请对 `Instantiate` 出的实例副本操作、别改共享资产（C7② 采用文档约束而非强制实例副本；见 `MD/design/重入安全-延迟队列方案.md`）。
