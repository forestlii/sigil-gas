...
- Demo smoke tests expanded from 1 to **9** (melee / lock-on / ranged / buff-stacking + input-dispatch / weapon-switch / focus-block / ranged-cancel / vehicle-horn), all green. They now ship **inside the sample** at `Samples~/PlayableDemo/Tests/`, so importing the Playable Demo brings runnable tests. Test totals: **EditMode 21 + PlayMode 100 = 121**.
