...
- **Ability activation is now reentrancy-safe (activation critical-section guard).** `GameplayAbility.IsActive` is only set inside `Activate()`, which runs *after* the cancel/block callbacks in `TryActivateSpec` (group-cancel, tag-relationship cancel → the cancelled ability's `OnEndAbility` user code). Re-entering `TryActivateAbility` on the *same* handle from one of those callbacks saw `IsActive` still false and slipped past the guard, double-activating the spec (loose activation tags applied twice, `OnActivateAbility` and `OnAbilityActivated` fired twice). A new `GameplayAbilitySpec.IsActivating` critical-section flag now covers that window; the re-entrant activation is rejected as `AlreadyActive`, mirroring Unreal's activation-reentrancy protection. Effects applied by the ability itself during activation (cost/cooldown) still apply immediately — this is a per-spec critical-section lock, not the effect deferral queue, so it deliberately does not defer them. *(Reentrancy stage 2 of 3 — input (C6/C7) still to come.)*
- **Input dispatch and buffer windows are now reentrancy-safe (stage 3 — C6/C7).** Two fixes: **(C7)** `InputControlSetup.HandleInput` iterated the shared `_filterScratch` list returned by `FilterInputProcessors`; a processor that re-dispatched input from inside its own `HandleInput` cleared that shared list mid-iteration, throwing `InvalidOperationException`. Filtering now borrows a per-call list from a pool (zero steady-state allocation), so nested dispatches use independent lists. **(C6)** `FireBufferedInput` closed *all* active buffer windows on completion — including one a processor opened during the dispatch — which broke combo pre-input chains; it now snapshots the windows present on entry and closes only those, leaving newly-opened windows alone. And `CloseInputBufferWindow` no longer double-dispatches the `(tag, false)` window-closed event when `FireBufferedInput` already closed that window. This completes the three reentrancy stages (effects, ability activation, input). *Note:* `InputControlSetup.AddChecker` / `AddProcessor` mutate the asset's serialized lists and are documented as setup-time only — for runtime changes, mutate an `Instantiate`d copy rather than the shared asset (C7② is handled by documentation rather than forced instance copies; see `MD/design/重入安全-延迟队列方案.md`).
