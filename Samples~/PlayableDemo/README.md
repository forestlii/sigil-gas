# Sigil · Playable Demo（可玩功能展示场）

一个**运行时构建**的可玩 demo，把 Sigil 已实现的多条战斗线串成一个场景给你直接玩：
**近战命中→扣血→GameplayCue（火花+受击闪红）、远程子弹、锁定切换、削韧破防、buff 叠层**，外加第三人称相机、敌人血条、体力消耗+回复、自解释 HUD。
...
| **Shift** | 冲刺 sprint |
| **鼠标 Mouse** | 看 look |
| **空格 / 左键 Space / LMB** | 近战 melee |
| **右键 / F RMB / F** | 远程子弹 ranged |
| **Tab** | 锁定 / 解锁 lock-on toggle |
| **Q / E** | 左 / 右切换锁定目标 switch target |
...
## 演示了哪些功能 What it shows

| 功能 Feature | 怎么看 How to see it |
|---|---|
| 近战 Melee + Cue | 空格/左键挥砍，命中敌人闪红 + 火花 |
| 远程子弹 Projectile | 右键/F 发射子弹（`BulletLauncher`）；锁定时自动朝目标 |
| 锁定 Lock-on | Tab 锁定最前方敌人，Q/E 在 3 个敌人间左右切换；HUD 显示当前目标 |
| 削韧破防 Poise | 持续攻击削减目标削韧条；归零→敌人变黄破防（HUD 显示削韧条 + ★破防）|
| buff 叠层 Stacking | 连按 R 叠加 Power（+MaxHealth/层）；HUD 显示 `×N` 层数 + 倒计时 |
| 可观测性 Observability | HUD 全靠订阅框架对外事件/只读枚举渲染——即"GAS 广播数据、UI 自接"的活演示 |
